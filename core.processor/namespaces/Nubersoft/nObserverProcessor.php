...
class nObserverProcessor implements nObserver
	{
		...
		public	function loadClientConfig()
			{
				$client_config = NBR_CLIENT_DIR._DS_.'settings'._DS_.'config-client.php';
		...
		public	function getCachedPrefs($dir)
			{
				...
				$dir		=	rtrim($dir,_DS_);	
				$xml		=	$conf($dir._DS_.'xml_add_list.json');
				$configs	=	$conf($dir._DS_.'configs.json');
				if(is_array($xml))
		...
		public	function setPresets()
			{
				...
				$settings['site']['error_404']			=	NBR_TEMPLATE_DIR._DS_."default"._DS_."site.error404.php";
				// Default 
				$settings['site']['template_folder']	=	NBR_TEMPLATE_DIR._DS_."default"._DS_;
				// Default header
				$settings['site']['template_head']		=	NBR_TEMPLATE_DIR._DS_."default"._DS_;
				// Layout for the prefs page template
				$settings['site']['system_prefs']		=	(!defined("SYS_PREFS_TEMP") || (defined("SYS_PREFS_TEMP") && !is_file(SYS_PREFS_TEMP)))? _DS_."core.ajax"._DS_."form.site.prefs.php":SYS_PREFS_TEMP;
				// Save folder for tempfiles
				$settings['site']['temp_folder']		=	(!defined("TEMP_DIR"))? NBR_ROOT_DIR._DS_.'..'._DS_.'temp'._DS_:NBR_ROOT_DIR.TEMP_DIR;
				// Save cache folder
				$settings['site']['cache_folder']		=	(!defined("CACHE_DIR"))? NBR_ROOT_DIR._DS_.'..'._DS_.'cache'._DS_:NBR_ROOT_DIR.CACHE_DIR;
				// This is overwritten on latter template retrieval. This is just default
				$settings['site']['template']			=	'default'._DS_.'template';
