			...
			public	static	function connect($settings = false)
				{
					...
					if(!$thisConn) {
						...
						\nApp::saveSetting('connection', array('database'=>false,'health'=>false));
						\nApp::saveError('connection', array('success'=>false,'error'=>'incomplete/invalid db creds'));
						return false;
					}
					...
					try {
						...
						if(is_object(self::$con))
							\nApp::saveSetting('connection', array('database'=>$creds['data'],'health'=>true));
						else
							\nApp::saveSetting('connection', array('database'=>$creds['data'],'health'=>false));
					}
			...
			public	static	function ValidateSettings($dbConn)
				{
					...
					if(!self::$valid) {
						\nApp::saveIncidental('database','Connection Error.');
						...
						if(is_admin()) {
							...
							if($_checkUsers == 0) {
								\nApp::saveIncidental('database', array('con_admin'=>'Database Failure on line '.__LINE__.'=>'.__FILE__));									
								if(is_file($inc = NBR_RENDER_LIB.'/classes/installer/connect.remote.php')) {
									include_once($inc);
									$_createTable	=	\FetchRemoteTable::Create(self::$nubsql,'all');
								}
								// $_createTable returns opposite of expected
								return	empty($_createTable);
							}
						}
						// Return failed
						return false;
