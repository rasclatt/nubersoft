	...
	class	Updater implements UpdateSys
		{
			...
			public	function getFiles($zipname = false,$tempFolder = 'temp/')
				{
					...
					$local	=	str_replace(_DS_._DS_,_DS_,$tempFolder);
					// Create the directory
					if(!is_dir($local))
						mkdir($local,0755,true);
					else {
							AutoloadFunction("FetchUniqueId");
							$local	=	mkdir(str_replace(_DS_._DS_,_DS_,rtrim($local,_DS_).FetchUniqueId()._DS_),0755,true);
						}
			...
			public	function moveFiles($supprErrs = true)
				{
					...
					foreach($getRoot as $subdir) {
						$iFile = $install_from._DS_.$subdir;
					...
					$install_from	=	str_replace(_DS_._DS_,_DS_,$install_from);
					...
					foreach($objects as $name => $object) {
						...
						$root_dir	=	str_replace(_DS_._DS_,_DS_,str_replace($install_from, $install_into._DS_, $name));
						if(is_dir($name)) {
							if(!is_dir($root_dir)) {
			...
			public	function MoveAndBackup($install_from = false, $install_into = false, $backuplocation = false)
				{
					...
					foreach($objects as $name => $object) {
						// Copy to file/folder destination
						$root_dir	=	str_replace(_DS_._DS_,_DS_,str_replace($install_from, $install_into, $name));
						if(is_dir($name)) {
							if(!is_dir($root_dir)) {
								if(!mkdir($root_dir, 0775)) {
									$this->final_report['error']['dir'][]	=	str_replace($install_into, "", $root_dir);
			...
			protected	function recurseCopy($path = false)
				{
					if(empty($path))
						return false;
			...
			protected	function copyToDest($objects = false)
				{
					if(empty($objects))
						return false;
