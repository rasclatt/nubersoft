	...
	class	ComponentEditor
		{
			...
			public	function AddNewComponent($CompSet = false)
				{
					register_use(__METHOD__);
					include(NBR_RENDER_LIB._DS_.'class.html'._DS_.'ComponentEditor'._DS_.'AddNewComponent.php');
				}
			...
			public	function Display($data = array(), $table = 'components', $allowAdd = true, $command = false)
				{
					...
					// 
					include(NBR_RENDER_LIB._DS_.'class.html'._DS_.'ComponentEditor'._DS_.'Display.php');
				}
			...
			public	function DeleteComponent($CompSet = false)
				{
					...
					if(isset($this->data['ID']) && !empty($this->data['ID']))
						include(NBR_RENDER_LIB._DS_.'class.html'._DS_.'ComponentEditor'._DS_.'DeleteComponent.php');
				}
			...
			public	function TinyMCE($CompSet = false)
				{
					register_use(__METHOD__);
					if(isset($this->data['ID']) && !empty($this->data['ID'])) {
						include(NBR_ROOT_DIR._DS_.'core.ajax'._DS_.'component'._DS_.'button.TinyMCE.php');
					}
				}
			...
			public	function HelpDesk($CompSet = false)
				{
					register_use(__METHOD__);
					if(isset($this->data['ID']) && !empty($this->data['ID'])) {
						include(NBR_ROOT_DIR._DS_.'core.ajax'._DS_.'component'._DS_.'button.helpdesk.php');
					}
				}
			...
			public	function DuplicateComponent($CompSet = false)
				{
					register_use(__METHOD__);
					include(NBR_RENDER_LIB._DS_.'class.html'._DS_.'ComponentEditor'._DS_.'DuplicateComponent.php');
				}
			...
			protected	function DuplicateComponentsList($values = false)
				{
					...
					if(!empty($values) && is_array($values)) {
							include(NBR_RENDER_LIB._DS_.'class.html'._DS_.'ComponentEditor'._DS_.'DuplicateComponentsList.php');
						}
				}
			...
			public	function	ContainerDropDown()
				{
					$query		=	nQuery();
					$_checkCol	=	$query->query("SHOW columns from `".$this->table."` where `field` = 'ref_page'")->getResults();
					...
					if($_checkCol != 0) {
						...
						include(NBR_RENDER_LIB._DS_.'class.html'._DS_.'ComponentEditor'._DS_.'ContainerDropDown.php');
					}
				}
		...
		public function createFormElements()
				{
					...
					foreach($layout as $compname => $compvals)
						include(NBR_RENDER_LIB._DS_.'class.html'._DS_.'ComponentEditor'._DS_.'createFormElements.php');
					...
					if(isset($_SESSION) && check_empty($_SESSION,'helpdesk','on')) {
						...
						if($check_helper != 0) {
							...
							if($check_helper !==  0) {
								...
								include(NBR_ROOT_DIR._DS_.'core.ajax'._DS_.'component'._DS_.'component.helpdesk.php');
							}
						}
					}
				}
