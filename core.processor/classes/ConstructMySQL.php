			...
			public	function fetch()
				{
					...
					if(nApp::getEngine('queries'))
						...
						nApp::saveSetting('engine',array('queries'=>1));
			...
			public	function tableExists($table = false, $type = 'post',$break = false)
				{
					...
					if($table) {
						\nApp::nFunc()->autoload('get_tables_in_db',NBR_FUNCTIONS);
			...
			public	function query($sql = false,$params = false)
				{
					...
					try {
						...
						$nFunctions->autoload('QuickWrite');
						$log	=	array("data"=>"sql_query: Authored by ".$author." / ".strip_tags(printpre($e->getMessage())),"dir"=>NBR_CLIENT_DIR."/settings/error_log/","filename"=>"sql_raw_errors.txt","mode"=>"c+");
						QuickWrite($log);
