			...
			public	function	component($inputArray, $table, $command, $displayCol = false)
				{
					...
					include(NBR_RENDER_LIB._DS_.'class.html'._DS_.'create'._DS_.'component.php');
					...
					if(isset($_SESSION['helpdesk']) && $_SESSION['helpdesk'] == 'on') {		$check_helper		=	$this->nubquery->addCustom("show tables like 'help_desk'",true);
						...
						if($check_helper !== 0) {
							...
							if($check_helper !==  0) {						$results	=	$check_helper[0]; 
								include(NBR_RENDER_LIB._DS_.'class.html'._DS_.'create'._DS_.'helpdesk.php');
							}
						}
					}
				}
			...
			public	function	dropMenu()
				{
					...
					if($_checkCol !== 0) {		
						...
						include(NBR_RENDER_LIB._DS_.'class.html'._DS_.'create'._DS_.'dropMenu.php');
					}
				}
			...
			public	function formAdd()
				{
					...
					include(NBR_RENDER_LIB._DS_.'class.html'._DS_.'create'._DS_.'formAdd.php');
				}
			...
			public	function dup_component()
				{
					...
					if(!empty($this->inputArray[0]['ID'])) 
						include(NBR_RENDER_LIB._DS_.'class.html'._DS_.'create'._DS_.'dup_component.php');
				}
			...
			public	function driveDownArr($curr)
				{
					...
					if(is_array($this->curr)) {		// Loop through values. If array, drive down into the array until single values are available
							...
							foreach($this->curr as $key => $value)
								include(NBR_RENDER_LIB._DS_.'class.html'._DS_.'create'._DS_.'driveDownArr.php');
						}
				}
			...
			public	function formDelete()
				{
					if(!empty($this->inputArray[0]['ID'])) {
						AutoloadFunction('FetchUniqueId');
						$rand	=	FetchUniqueId();
						include(NBR_RENDER_LIB._DS_.'class.html'._DS_.'create'._DS_.'formDelete.php');
					}
				}
			...
			public	function formTinyMCE()
				{
					...
					if($checkBox['page_live'] == 'on') {					
						...
						include(NBR_RENDER_LIB._DS_.'class.html'._DS_.'create'._DS_.'formTinyMCE.php');	
					}
				}
...
//=======> FUNCTION: ADD HELP
			...
			public	function formHelpDesk()
				{
					include(NBR_RENDER_LIB._DS_.'class.html'._DS_.'create'._DS_.'formHelpDesk.php');
				}
			...
			public function createFormElements()
				{
					...
					include(NBR_RENDER_LIB._DS_.'class.html'._DS_.'create'._DS_.'createFormElements.php');
				}
		}