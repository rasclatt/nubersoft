	...
	class	DBWriter
		{
			...
			public	function execute($payload = false)
				{
					...
					if($nubquery && in_array(1,$_checkAction)) {
						...
						if(!empty($_table)) {// && array_sum($_allow) >= 1
							...
							\nApp::nFunc()->autoload('upload_file',NBR_FUNCTIONS);
			...
			protected	function writeFileInstance($files_to_add = false,$type = 'insert')
				{ 
					...
					$query		=	nQuery();
					// Assign Table
					$table		=	(!empty($this->table))? $this->table : nApp::getDefaultTable();
					...
					$this->cols_in_table	=	nApp::getColumns($table);
					...
					if(!empty($query->err)) {
						...
						$errs	=	Safe::to_object($query->err);
						$var	=	(!empty($errs->connection->{2}))? $errs->connection->{2} : false;
						nApp::saveIncidental('nQuery', array('success'=>'fail','error'=>printpre($query)));//filter_error_reporting($var)
						$log	=	array("data"=>"SQL: Authored by ".$author." / ".__FILE__."->".__LINE__.PHP_EOL.'ERRORS: '.strip_tags(printpre($query->errors)),"dir"=>NBR_CLIENT_DIR."/settings/error_log/","filename"=>"sql_errors.txt","mode"=>"c+");
					}
