	...
	class	BuildCache
		{
			...
			public	function Initialize($fileOutput = 'html')
				{
					...
					if($this->autoCache) {
						...
						if($this->allow) {
							...
							$reqfilename		=	nApp::getPage("ID");
							$this->fileOutput	=	$fileOutput;
							$cacheroot			=	(!empty(nApp::getSite("cache_folder")))? nApp::getSite("cache_folder") : false;
							$this->cacheRoot	=	(!empty($cacheroot))? $cacheroot: ((defined("CACHE_DIR"))? CACHE_DIR : NBR_CLIENT_DIR.'/settings/cache/');
							...
							$this->cacheDir		.=	(!empty($_SESSION['ID']))? _DS_.$_SESSION['ID']:"";
							// DocumentRoot/cache/$ID/$_SESSION/$ID.ext
							$this->cachefile	=	str_replace(_DS_._DS_,_DS_,$this->cacheDir._DS_.'index.'.$this->fileOutput);
					...
					if($this->data) {
						if((isset($this->_cache) && !$this->_cache) && ($this->allow)) {
							...
							if($fp) {
								...
								QuickWrite(array("data"=>"Cache failed".PHP_EOL."Likely Permissions".PHP_EOL,"dir"=>NBR_CLIENT_DIR."/settings/error_log/","filename"=>"cache.log.txt","skip_post"=>true,"mode"=>"c+"));
							}
						}
					}
			...
			public	function checkCacheFile($filename = false)
				{	
					...
					if(directory_exists($fInfo['dirname'],true) && is_file($this->filename)) {
			...
			public	function renderBlock()
				{
					...
					write_file(array("save_to"=>$this->filename, "content"=>$this->data));
				}
