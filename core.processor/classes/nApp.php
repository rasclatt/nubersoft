		...
		public	static	function saveToLogFile($filename = false,$message = false)
			{
				...
				write_file(array("save_to"=>str_replace(_DS_._DS_,_DS_,NubeData::$settings->site->temp_folder._DS_."{$filename}"),"content"=>json_encode(array('debug'=>debug_backtrace(),'message'=>$message))));
			}
		...
		public	static	function stripRoot($value = false,$addSite = false)
			{
				...
				return ($addSite)? str_replace(_DS_._DS_,_DS_,site_url().$value) : $value;
			}
		...
		public	static	function getConfigs($location = false)
			{
				...
				$parser	->addLocation(NBR_CLIENT_DIR._DS_.'settings')
						->addLocation(NBR_CLIENT_DIR._DS_.'workflows')
						->addLocation(NBR_CLIENT_DIR._DS_.'apps')
						->addLocation(NBR_TEMPLATE_DIR._DS_.'default')
						->addLocation(NBR_CORE._DS_.'workflows')
						->addLocation(NBR_CLIENT_DIR._DS_.'template')
						->addLocation(NBR_ROOT_DIR._DS_.'core.plugins');
		...
		public	static	function getUploadDir($table,$settings = false)
			{
				...
				return ($append)? str_replace(_DS_._DS_,_DS_,NBR_ROOT_DIR._DS_.$path) : $path;
			}
	}