			...
			protected	static	function RenderIterator($current, $key = '')
				{
					...
					$_layout	=	self::$RenderEngine	->SetStyles()
														->SetIdClass('_id')
														->SetIdClass('class')
														->Compile()
														->display_inline;
			...
			public	static	function ComponentEditors($component_settings = array())
				{
					...
					if(!is_file($custom = NBR_CLIENT_DIR._DS_.'components'._DS_.'template'._DS_.'page'._DS_.'index.php'))
						include(NBR_RENDER_LIB._DS_.'class.html'._DS_.'core'._DS_.'ComponentEditors.php');
					else
						include($custom);
				}
			/*
			...
			private	static	function getCompSettings($curr,$unique_id)	
				{
					...
					if(!empty($curr['component_type'])) {
						if(is_file(NBR_ROOT_DIR._DS_.'core_images'._DS_.'core'._DS_.'icn_'.$curr['component_type'].'.png'))
							$icon	=	site_url().'/core_images/core/icn_'.$curr['component_type'].'.png';
						// Convert icon to base 64
						elseif(is_file($icFile = NBR_CLIENT_DIR._DS_.'components'._DS_.$curr['component_type']._DS_.'icon.png')) {
								AutoloadFunction('base64image');
								$icon	=	base64image($icFile);
							}
					}
					...
					if(!empty($curr['file_path'])) {
						if(is_file(NBR_ROOT_DIR.$curr['file_path'].$curr['file_name'])) {
							$use	=	(is_file(str_replace(_DS_._DS_,_DS_,NBR_THUMB_DIR._DS_."components"._DS_.$curr['file_name'])))? str_replace(NBR_ROOT_DIR,"",NBR_THUMB_DIR."/components/") : $curr['file_path'];	
							$bImg	= 'background-image: url('.site_url().str_replace(_DS_,'/',$use).$curr['file_name'].'); background-repeat: no-repeat; background-size: cover;';
						}
					}
			...
			public	static	function Footer($settings = false)
				{	
					...
					if($toggle == 'on') {
						...
						include(NBR_TEMPLATE_DIR._DS_.'default'._DS_.'foot.php');
					}
					...
					if(!empty($_error)) {
						...
						include(NBR_RENDER_LIB._DS_.'class.html'._DS_.'core'._DS_.'Footer.js.php');
					}
				}
			...
			public	static	function Header($header = false,$_bypass = false)
				{
					...
					include(NBR_RENDER_LIB._DS_.'class.html'._DS_.'core'._DS_.'Header.php');
