...
<?php 	// Delete cache
		$dir		=	(defined("CACHE_DIR"))? str_replace("//","/",NBR_ROOT_DIR.'/'.CACHE_DIR) : nApp::getSite('cache_folder');
		$directory	=	(is_dir($dir));
		$hasCache	=	$checkFolder($dir);
		
		if(empty($dir))
			echo 'Cache folder not set. You must define it in the registry.xml file, then refresh the DEFINES: /client/settings/registry.xml';
