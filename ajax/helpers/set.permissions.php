<?php
	...
	class	RolesEngine
			...
			public	function __construct()
				{
					...
					autoload_function('process_requests,default_jQuery,check_empty');
		...
		<form id="permissions" method="post" class="submit-form">
			...
			autoload_function('get_tables_in_db');
	...
	// Autoload functions
	autoload_function('default_jQuery,check_empty,organize,get_user_roles,nquery');
	...
	if($all_defs != 0) {
			...
			for($a = 0; $a < $aDefsCnt; $a++) {
				 ?>
	<div class="cursorPointer" onClick="AjaxFlex('#form-loader','/core/ajax.engine/set.permissions.php?id=<?php echo $all_defs[$a]['ID']; ?>&mode=focus')"><?php echo "(".$all_defs[$a]['usergroup'].") ".$all_defs[$a]['content']['description']; ?></div>
			<?php
				}
		}
	//echo default_jQuery();
...
$("#save_new").on("change",function(){
	...
	function SaveForm(ElemId)
		{
			$.ajax({
					url:	'/core/ajax.engine/set.permissions.php',
