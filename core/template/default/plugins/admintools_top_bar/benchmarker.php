...
$(document).ready(function() {
	...
	$('#nbr_load_time').html('<a href="<?php echo $this->localeURL($this->getPageURI('full_path').'?automate='.$this->getHelper('nToken')->nOnceClear('workflow_runtime_total')->nOnce('workflow_runtime_total',json_encode(array('action'=>'download','file'=>$this->getCacheFolder($name),'usergroup'=>'NBR_SUPERUSER')))->getToken()) ?>">'+loadTime+'secs</a>');
	if(loadTime > 2)
		classTime	=	'badtime';
	else if(loadTime <= 1)
		classTime	=	'goodtime';
