...
<div>
	...
	$select	=	organize($nubquery	->select(array('column_name','column_type','size','restriction'),true)
									...
									->getResults(),'column_name');
									
	// Check if this table has a custom table layout
	$this->autoload('CustomTableLayout');
	$temp	=	CustomTableLayout($this->getGlobalArr('engine','table_name'),$this->data['results'],$this->columns,$select,$dropdowns);
