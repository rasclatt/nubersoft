...
if(!is_admin())
	...
	function DisplayFiles($values,$columns,$dropdowns)
		{
			nApp::call()->autoload('form_field,get_file_extension');
...
$nProcToken	=	self::call('nToken')->setMultiToken('nProcessor','imagebucket');
		...
		<form enctype="multipart/form-data" method="post">
			...
			if(is_file(NBR_ROOT_DIR.$file)) {
					...
					<video controls height="100">
						<source src="<?php echo $row['file_path'].self::call('Safe')->encode($row['file_name']); ?>" type="video/<?php echo str_replace(".","",$ext[0]); ?>">
					</video>
						<?php }
					elseif(preg_match('/.mov|.mpg|.mpeg|.avi|.m4v/i',$row['file_name'],$ext)) { ?>
							<embed src="<?php echo $row['file_path'].self::call('Safe')->encode($row['file_name']); ?>"></embed><?php
						}
					else {
						 ?>
