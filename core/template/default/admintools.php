...
include(__DIR__.DS.'inclusions'.DS.'config.admin.php');
...
echo default_jQuery();
echo render_element_js(NBR_ROOT_DIR.DS.'js'.DS,false);
echo render_element_js(NBR_ROOT_DIR.DS.'plugins'.DS);
echo render_element_js(NBR_CLIENT_DIR.DS.'js'.DS,false);
echo render_element_css(NBR_ROOT_DIR.DS.'css'.DS);
echo render_element_css(NBR_ROOT_DIR.DS.'plugins'.DS);
echo (new \Nubersoft\nView())->getStyles();
$cache	=	nApp::cacheEngine();
// If there is a header file, just get it. If not, make it
//$cache->checkCacheFile(nApp::getCacheFolder().DS.'admin'.DS.'header.html')->startCaching();
...
<?php echo render_masthead(); ?>
	...
	if(is_admin()) { ?>
		...
		<?php
			...
			$settings['layout']		=	DS."core".DS."renderlib".DS."admintools.pagination.results.php";
