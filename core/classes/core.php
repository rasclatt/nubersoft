			...
			public	static	function ComponentEditors($component_settings = array())
				{
					...
					if(!is_file($custom = NBR_CLIENT_DIR.DS.'components'.DS.'template'.DS.'page'.DS.'index.php'))
						include(NBR_RENDER_LIB.DS.'class.html'.DS.'core'.DS.'ComponentEditors.php');
					else
						include($custom);
				}
			/*
			...
			private	static	function getCompSettings($curr,$unique_id)	
				{
					...
					if(!empty($curr['component_type'])) {
						if(is_file(NBR_ROOT_DIR.DS.'images'.DS.'core'.DS.'icn_'.$curr['component_type'].'.png'))
					...
					if(!empty($curr['file_path'])) {
						if(is_file(NBR_ROOT_DIR.$curr['file_path'].$curr['file_name'])) {
							...
							$bImg	= 'background-image: url('.site_url().str_replace(DS,'/',$use).$curr['file_name'].'); background-repeat: no-repeat; background-size: cover;';
						}
					}
			...
			public	static	function Footer($settings = false)
				{	
					...
					if($toggle == 'on') {
						...
						include(NBR_TEMPLATE_DIR.DS.'default'.DS.'foot.php');
					}
					...
					if(!empty($_error)) {
						...
						include(NBR_RENDER_LIB.DS.'class.html'.DS.'core'.DS.'Footer.js.php');
					}
				}
			...
			public	static	function Header($header = false,$_bypass = false)
				{
					...
					include(NBR_RENDER_LIB.DS.'class.html'.DS.'core'.DS.'Header.php');
