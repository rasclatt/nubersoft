			...
			public	function renameFile($curr_path,$file_name,$duplicate = false)
				{
					...
					$getMime	=	(!empty($exp[0]) && !empty($exp[1]))? strtolower(implode(DS,array(preg_replace('/[^a-zA-Z0-9]/','',$exp[1]),$exp[0]))) : false;
					$ext		=	pathinfo($curr_path,PATHINFO_EXTENSION);
					$file_path	=	pathinfo($curr_path,PATHINFO_DIRNAME);
					$file_name	=	preg_replace('![^'.DS.'/a-zA-Z0-9\.\-\_]!','',pathinfo($file_name,PATHINFO_FILENAME));
					
					$oldName	=	$curr_path;
					$newName	=	str_replace(DS.DS,DS,$file_path.DS.$file_name.'.'.$ext);
					...
					if($this->_fileArray !== 0) {
						...
						if($_ValidDest == 1) {
							...
							if(check_empty($this->payload,'thumbnail','1')) {
								...
								$thumbdir		=	(!defined('NBR_THUMB_DIR'))? NBR_ROOT_DIR.DS.'client_assets'.DS.'thumbs'.DS:NBR_THUMB_DIR."/";
								$thumbdir		=	$thumbdir.DS.$useTable.DS;
								if(!is_dir($thumbdir))
