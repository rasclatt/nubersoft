	...
	class	ComponentEditor
		{
			...
			public	function AddNewComponent($CompSet = false)
				{
					register_use(__METHOD__);
					include(NBR_RENDER_LIB.DS.'class.html'.DS.'ComponentEditor'.DS.'AddNewComponent.php');
				}
			...
			public	function Display($data = array(), $table = 'components', $allowAdd = true, $command = false)
				{
					...
					// 
					include(NBR_RENDER_LIB.DS.'class.html'.DS.'ComponentEditor'.DS.'Display.php');
				}
			...
			public	function DeleteComponent($CompSet = false)
				{
					...
					if(isset($this->data['ID']) && !empty($this->data['ID']))
						include(NBR_RENDER_LIB.DS.'class.html'.DS.'ComponentEditor'.DS.'DeleteComponent.php');
				}
			...
			public	function TinyMCE($CompSet = false)
				{
					register_use(__METHOD__);
					if(isset($this->data['ID']) && !empty($this->data['ID'])) {
						include(NBR_ROOT_DIR.DS.'ajax'.DS.'component'.DS.'button.TinyMCE.php');
					}
				}
			...
			public	function HelpDesk($CompSet = false)
				{
					register_use(__METHOD__);
					if(isset($this->data['ID']) && !empty($this->data['ID'])) {
						include(NBR_ROOT_DIR.DS.'ajax'.DS.'component'.DS.'button.helpdesk.php');
					}
				}
			...
			public	function DuplicateComponent($CompSet = false)
				{
					register_use(__METHOD__);
					include(NBR_RENDER_LIB.DS.'class.html'.DS.'ComponentEditor'.DS.'DuplicateComponent.php');
				}
			...
			protected	function DuplicateComponentsList($values = false)
				{
					...
					if(!empty($values) && is_array($values)) {
							include(NBR_RENDER_LIB.DS.'class.html'.DS.'ComponentEditor'.DS.'DuplicateComponentsList.php');
						}
				}
			...
			public	function	ContainerDropDown()
				{
					...
					if($_checkCol != 0) {
						...
						include(NBR_RENDER_LIB.DS.'class.html'.DS.'ComponentEditor'.DS.'ContainerDropDown.php');
					}
				}
		...
		public function createFormElements()
				{
					...
					foreach($layout as $compname => $compvals)
						include(NBR_RENDER_LIB.DS.'class.html'.DS.'ComponentEditor'.DS.'createFormElements.php');
					...
					if(isset($_SESSION) && check_empty($_SESSION,'helpdesk','on')) {
						...
						if($check_helper != 0) {
							...
							if($check_helper !==  0) {
								...
								include(NBR_ROOT_DIR.DS.'ajax'.DS.'component'.DS.'component.helpdesk.php');
							}
						}
					}
				}
