...
class	NuberEngine
	{
		...
		public	static	function callPlugin($name)
			{
				if($name == 'core')
					return new PluginEngine(__DIR__.DS.'..'.DS.'engine'.DS.'plugins');
			}
		/*
						...
						if(isset($data[$type])) {
							...
							if(!empty($data[$type]['classes'])) {
								...
								foreach($data[$type]['classes'] as $class => $vals) {
									...
									if(!empty($vals['dir']) && is_file($inc = str_replace(DS.DS,DS,NBR_ROOT_DIR.DS.$vals['dir'].DS.$class.".php")))
									...
									$instructions[str_replace(DS,"_",$instr)]	=	array("instr"=>$instr,
																							"methods"=>$methods,
																							"type"=>"class",
																							"name"=>$class);
								}
							...
							if(!empty($data[$type]['includes'])) {
								array_map(function($v){
										$file = str_replace(DS.DS,DS,NBR_ROOT_DIR.DS.$v);
										if(is_file($file))
											include($file);
