			...
			public	function errorFeedback($var,$type = 'unknown',$is_error = false)
				{
					...
					switch($type) {
							case('url_invalid') : 
								$offending	=	(!is_file($actFile = str_replace(DS.DS,DS,NBR_ROOT_DIR.DS.$var)))? str_replace(DS.DS,DS,$root.str_replace(NBR_ROOT_DIR,"",$actFile)) : false;
								$color		=	"orange";
								if($offending != false)
									$this->validation['warning'][]		=	$type;
								break;
							case('url_dynamic') :
								$offending	=	(preg_match('/\.php$|\.phtml$|\.php3$|\.asp$/i',$var))? false : str_replace(DS.DS,DS,$root.$var);
								$color		=	"red";
								if($offending != false)
