	...
	class BuildDatabase
		{
			...
			public	function __construct($_creds = array(), $classes_path = '/core/', $db_path = '/dbconnect.php', $curr_path='/')
				{
							...
							if(isset($_POST['setup']) && $_POST['setup'] == 'run') {
								...
								if(!empty($response)) {
									...
									if((is_dir($this->curr_path . '/') && $_REQUEST['consume'] == 'on') && (str_replace("/","",$this->curr_path) !== str_replace("/","",$_SERVER['DOCUMENT_ROOT']))) {
										...
										include_once($this->curr_path.DS.'classes'.DS.'directory.engines'.DS.'recursive.delete.php');
										$deleteQB	=	new recursiveDelete;
										$deleteQB->delete($this->curr_path.DS);
										exit;
									}
								}
							else { ?>
