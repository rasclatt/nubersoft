		...
		if(is_array($directoryExp)) {
			foreach($directoryExp as $keys => $values){
				...
				if(!$this->isDir($resetCachRoot))
					$this->toMsgAdminCoreAlert('Could not make dir: '.$resetCachRoot);

				if(!$this->isDir($resetCachRoot . $buildDirRoots))
					$this->toMsgAdminCoreAlert('Could not make dir: '.$resetCachRoot . $buildDirRoots);
			}
		}
		else {
			$path	=	str_replace(DS.DS,DS,$resetCachRoot.DS.$directoryExp);
			if(!$this->isDir($path))
				$this->toMsgAdminCoreAlert('Could not make dir: '.$path);
		}
