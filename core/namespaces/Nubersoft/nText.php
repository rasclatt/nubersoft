...
class nText
	{
		...
		**	@resource		http://php.net/manual/en/pspell-new-personal.php
		*/
		...
		public	function createLib($settings = false)
			{
				...
				if($pers)
					...
					throw new \Exception($this->errorMsg('1004',array('personal list','http://php.net/manual/en/pspell-new-personal.php')));
				// Use replacement pairs
				if($repl)
					pspell_config_repl($custom, $repl);
				else
					throw new \Exception($this->errorMsg('1004',array('replacement pairs list','http://php.net/manual/en/pspell-new-personal.php')));
				// New resource
		...
		public	function savePersonalFile($dir,$filename,$lang = 'en')
			{
				...
				pspell_config_personal($config, str_replace(DS.DS,DS,$dir.DS.$filename.'.pws'));
