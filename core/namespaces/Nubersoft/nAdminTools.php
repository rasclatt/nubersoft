...
class	nAdminTools	extends nConfigEngine
	{
		...
		public	function __construct(\Nubersoft\configFunctions $configFunctions)
			{
				$this->nApp		=	nApp::call();
				return parent::__construct($configFunctions);
			}
		...
		public	function useConfigs($dir = false)
			{
				...
				$this->cEngine	=	new configFunctions(new nAutomator($this->nApp));
		...
		public	function includeByTableName()
			{
				...
				foreach($this->layout['ontable'] as $includer) {
					if(isset($includer[0])) {
						...
						$this->nameLoop($includer);
				}
			}
		
		private	function nameLoop($includer)
			{
				if(!empty($includer['name'])) {
					...
					if($type) {
						$inc = $this->getHelper('nAutomator',$this->nApp)->matchFunction($includer[$type]);
						try {
							if(is_file($inc)) {
