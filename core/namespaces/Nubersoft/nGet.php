...
class nGet extends \Nubersoft\nFunctions
{
	...
	*/
	public	function issetCoreElement($type)
	{
		...
		return (isset(parent::$settings->{$type}));
	}
	...
	public	function getCoreElement($type = false)
	{
		if(empty($type)) {
			...
			return (!empty(parent::$settings))? parent::$settings : false;
		}
		...
		switch($type) {
			...
			//		return NubeData::$settings->{$type};
				elseif(isset(parent::$settings->{$type}))
					return parent::$settings->{$type};
		}
	...
	public	function getSetSitePrefs($refresh = false)
	{
		...
		if(!$nubquery) {
			...
			$this->setDataAttr('critical',array('nubquery_engine'=>false));
			$this->setDataAttr('database',array('nubquery_engine'=>false));	
			return false;
		}
		...
		$this->setDataAttr('preferences',$prefs);
	...
	public	function getColumns($table)
	{
		...
		$this->setDataAttr('columns_in_'.$table, $cols);
	...
	public	function getColumnInfo($table = false,$cols = false)
	{
		...
		if(!is_array($cols)) {
			if($table)
				...
				$this->setDataAttr('col_attr_in_'.$table, $cols);
		}
		else {
			$new	=	array();
			...
			$this->setDataAttr('col_attr_in_'.$table, $new);
	...
	public	function getRoutingTables()
	{
		...
		$this->setDataAttr('routing_tables',$tables);
	...
	public	function getPageURI($def = false,$reqUrl = false)
	{
		...
		if(empty($this->getDataAttr($nodeName))) {
			...
			$this->setDataAttr($nodeName,$base);
		}
		else
			$base	=	$this->toArray($this->getDataAttr($nodeName));
		# If path is found			
		...
		$this->setDataAttr('pageURI',$result);
		# Return the results
		return $result;
	}
	...
	public	function getAdminPage()
	{
		...
		$this->setDataAttr('admin_page',$setting);
		return $setting;
	}
	...
	public	function getDropDowns($table = false)
	{
		...
		$this->setDataAttr('dropdowns_'.$table,$new);
	...
	public	function getFormBuilder($columns = false,$gCols = array("column_name","column_type","size"),$org = 'column_name')
	{
		...
		$this->setDataAttr('form_builder',$cols);
	...
	public	function getAllMenus()
	{
		...
		$this->setDataAttr('all_menus',$menus);
