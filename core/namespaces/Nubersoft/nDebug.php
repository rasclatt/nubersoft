		...
		public	function printPre($print = false,$settings = false)
		{
			...
			if($backtrace) {
				...
				$debugBlock	=	implode('<br />'.PHP_EOL,array_map(function($v){
					...
					return (!empty($v['file']))? '<b style="margin: 0 0 5px 0; color: blue;">'.$class.$v['function'].'()</b> <span style="color: #666;">– '.str_replace(NBR_ROOT_DIR, '', $v['file']).'</span> <span  style="color: red;">('.$v['line'].')</span>' : '<b style="margin: 0 0 5px 0; color: #666;">'.$v['function'].'('.implode(', ',$v['args']).')</b>';
				},$debug));
			}
