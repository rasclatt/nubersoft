	...
	*/
	...
	public	function getCatelogueByCategory($on = false)
	{
		...
		return $this->getPrefFile('cart_products_category_'.$LOCALE,array('save'=>true),false,function($path,$nApp) use ($on,$LOCALE) {
			$all	=	$nApp->nQuery()->query("SELECT *, CONCAT(file_path,file_name) as image_path FROM cart_products WHERE {$on} `product_sku` IN (select `product_sku` from `cart_products_locales` WHERE `locale_abbr` = '{$LOCALE}') ORDER BY page_order ASC")->getResults();
