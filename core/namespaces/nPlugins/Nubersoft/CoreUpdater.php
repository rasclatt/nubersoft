...
class CoreUpdater extends \Nubersoft\nRouter
{
	public	function getGitFile()
	{
		...
		if(!$this->isDir(pathinfo($endpoint,PATHINFO_DIRNAME))) {
			$this->toMsgAdminCoreAlert('Save folder could not be created to move update to.');
			return;
		}
		...
		if(is_file($endpoint))
			...
			$this->toMsgAdminCoreAlert('Update file was not found. Error probably occurred saving or retieving file.');
	}
}