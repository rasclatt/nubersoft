...
class RenderElements extends \nPlugins\Nubersoft\CoreHelper
	{
		...
		public	function renderURLElement($applyURL)
			{
				...
				if(!empty($this->applyURL)) {
					$this->renderURL[0]			=	'<a href="'.$this->safe()->decode($this->applyURL).'" title="'.$this->safe()->decode($this->applyURL).'">';
					$this->renderURL[1]			=	'</a>';
				}
			}
		...
		public	function renderContentElements($payload, $filterArray)
			{
				$unique_id	=	$this->getPageURI('unique_id');
				...
				if(!empty($this->payload['content']))
					$this->payload['content']	=	$this->safe()->decode($this->payload['content']);
				...
				if(isset($this->payload['component_type']) && $this->payload['component_type']	== 'text') {
						...
						$this->returnFinal[0]		=	"<$setStyle ".$this->payload['class'].' style="'.$this->finalStyle.'">'.$this->safe()->decode($this->payload['content'])."</$setStyle>";
					}
				...
				elseif(isset($this->payload['component_type']) && $this->payload['component_type']	== 'image') {
					if(isset($this->payload['file_path']) && !empty($this->payload['file_path'])) {
						...
						$file_check_dir				=	($file_check !== 0)? str_replace($_SERVER['DOCUMENT_ROOT'], "", $file_check_res['file_path']): '/client/images/default/';
				...
				elseif($this->checkEmpty($this->payload,'component_type','button'))
						$this->returnFinal[0]		=	'<a '.$this->payload['class'].' href="'.$this->safe()->decode($this->payload['a_href']).'" style="'.$this->finalStyle.'">'.$this->payload['content'].'</a>';
						
				// Set rules for EMAIL INPUT
				elseif($this->checkEmpty($this->payload,'component_type','form_email')) { 
		...
		public	function renderFinal($payload, $style)
			{
				// Sets the array var
				$this->payload			=	$payload;
				$this->style			=	$style;
				...
				// Set rules for TEXT INPUT
				if($this->checkEmpty($this->payload,'component_type','text')) {
					$setStyle	=	(empty($this->style))? 'span': 'div';
					$this->returnFinal[0]		=	"<$setStyle ".$this->payload['class'].' style="'.$this->style.'">'.$this->safe()->decode($this->payload['content'])."</$setStyle>";
				}
				// Set rules for CODE INPUT
				elseif($this->checkEmpty($this->payload,'component_type','code')) {
					$this->returnFinal[0]		=	'<div '.$this->payload['class'].' style="'.$this->style.'">'.$this->payload['content'].'</div>';
				}
				// Set rules for IMAGE INPUT
				elseif($this->checkEmpty($this->payload,'component_type','image')) {
					if(!empty($this->payload['file_path'])) {
						...
						$file_check_res	=	$file_check[0];
						$file_check_dir	=	($file_check !== 0)? str_replace(NBR_ROOT_DIR, "", $file_check_res['file_path']): '/client/images/default/';
						$this->returnFinal[0]		=	'<img src="'.$file_check_dir.$file_check_res['file'].'"'.$this->payload['class'].' style="'.$this->style.'" />';
					}
				}
				// Set rules for BUTTON INPUT
				elseif($this->checkEmpty($this->payload,'component_type','button'))
						$this->returnFinal[0]		=	'<a '.$this->payload['class'].' href="'.$this->safe()->decode($this->payload['a_href']).'" style="'.$this->style.'">'.$this->payload['content'].'</a>';
		...
		public	function simpleEmailer()
			{
				if($this->checkEmpty($this->payload,'page_live','on')) {
	...
	<div class="formButton"><input disabled="disabled" type="submit" value="SEND"></div>
									...
									$defEmail	=	$nubsql->getResults("select * from emailer where email_id = '$email_id' and page_live ='on'");
									if($defEmail !== 0) {
											include_once(NBR_ROOT_DIR. DS."core".DS.'includes'.DS.'classes'.DS.'browser.commands'.DS.'string.include.php');
											$result		=	$defEmail[0];
											...
											$message		=	$this->safe()->encode($_REQUEST['question']);
										}
								   ...
								   $render_array = (isset($result['return_response']) && !empty($result['return_response']))? $this->safe()->decode($result['return_response']): "<h2>Thank you for using our mail form</h2>";
								}
							}
						else {
							$render_array = '
