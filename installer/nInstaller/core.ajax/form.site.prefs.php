			...
			<div class="nbr_hidewrap">
				...
				<div class="nbr_prefpanel_content" id="nbr_site_pane">
					...
					<form id="uploadFAVICON" method="post" enctype="multipart/form-data" action="/core.ajax/ajax.dispatcher.php">
						<input type="hidden" name="token[ajax_edit_favicon]" value="<?php AutoloadFunction("fetch_token"); echo fetch_token('ajax_edit_favicon'); ?>" />
					...
					<form id="settings_site" method="post" action="<?php echo $_SERVER['HTTP_REFERER']; ?>">
						...
						$dirs	=	get_directory_list(array("dir"=>CLIENT_DIR.'/template/'));
						if(!empty($dirs['dirs'])) {
						?>
						<label><div>Primary Template</div>
							<select name="content[template_folder]">
								<option value="<?php echo $basedir = Safe::encode(str_replace(ROOT_DIR,"",TEMPLATE_DIR."/default")); ?>" <?php if(!empty($site->template_folder) && $site->template_folder == $basedir) echo " selected"; ?>><?php echo basename($basedir); ?></option>
							<?php
									foreach($dirs['dirs'] as $folder) {
											$trimmed	=	rtrim($folder,"/");
							?>
								<option value="<?php echo $thisDir = Safe::encode(str_replace(ROOT_DIR,"",$trimmed)); ?>"<?php if(!empty($site->template_folder) && $site->template_folder == $thisDir) echo " selected"; ?>><?php echo basename($thisDir); ?></option>
							<?php
										}
								}
							?>
						...
						<label>	Login Bypass
							<div class="form-input">
								<input type="text" name="content[login]" value="<?php echo (isset($site->login))? $site->login:""; ?>" placeholder="You can bypass the normal login process" />
							</div>
						</label>
						<label>	Head Bypass
							<div class="form-input">
								<input type="text" name="content[head]" value="<?php echo (isset($site->head))? $site->head:""; ?>" placeholder="You can bypass the template mast head here" />
							</div>
						</label>
						<label>	Menu Bypass
							<div class="form-input">
								<input type="text" name="content[menu]" value="<?php echo (isset($site->menu))? $site->menu:""; ?>" placeholder="You can bypass the template menu here" />
							</div>
						</label>
						<label>	Foot Bypass
							<div class="form-input">
								<input type="text" name="content[foot]" value="<?php echo (isset($site->login))? $site->foot:""; ?>" placeholder="You can bypass the template footer here" />
							</div>
						</label>
...
?>
				...
				<div class="nbr_prefpanel_content" id="nbr_head_pane" style="display: none;">
					...
					<form id="settings_head" method="post" action="<?php echo $_SERVER['HTTP_REFERER']; ?>">
						...
						<label>	Javascript
							<div class="form-input">
								<textarea name="content[javascript]" placeholder="Create JavaScript links" class="textarea" ><?php echo (isset($header->javascript))? $header->javascript:""; ?></textarea>
							</div>
						</label>
						<label>	TinyMCE
							<div class="form-input">
