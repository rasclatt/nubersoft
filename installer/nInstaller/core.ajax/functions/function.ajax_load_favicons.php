	...
	function ajax_load_favicons()
		{
			...
			$imgs	=	get_directory_list(array("dir"=>ROOT_DIR,"type"=>array("ico","png"),"recursive"=>false));
			...
			foreach($imgs['root'] as $icns) {
				if(preg_match('/favicon\./',$icns))
					$fImg[]	=	'<img src="'.site_url().$icns.version_from_file(ROOT_DIR.$icns).'" style="margin: 5px; height: 40px; width: 40px; border: 1px solid #FFF; box-shadow: 1px 1px 8px #000;" />';
			}
