	...
	if(check_empty($REQUEST,'command','forgot_pass')) {
			...
			if(filter_var($REQUEST['email'],FILTER_VALIDATE_EMAIL)) {
					...
					if($email !== false && filter_var($email,FILTER_VALIDATE_EMAIL)) {
							...
							include(RENDER_LIB."/assets/login/message.{$errMsg}.php");
			...
			if(!isset($messageinclude))
				include(RENDER_LIB.'/assets/login/message.invalid.php');
		}
	else
		include(RENDER_LIB.'/assets/login/message.reset.php');