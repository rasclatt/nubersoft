<?php
	...
	if(empty($_GET['table']) && empty($_POST['requestTable']))
		return;
	...
	if(isset($_POST['change_name']) || isset($_POST['file_name'])) {
			...
			$fullpath			=	ROOT_DIR.$local;
			$nfullpath			=	$fullpath.$_POST['file_name'];
			
			if(!isset($_POST['change_name'])) {
					AutoloadFunction('get_directory_list');
					$valid		=	get_directory_list(array("dir"=>ROOT_DIR.$_POST['file_path']));
				...
				if(!empty($_POST['file_name'])) {
						...
						if(is_file($fullpath.$_POST['old_name']) && rename($fullpath.$_POST['old_name'],$fullpath.$_POST['file_name'])) {
								if(defined('THUMB_DIR')) {
										$thumbdir		=	THUMB_DIR."/".$_POST['requestTable']."/";
										$thumb 			=	$thumbdir.$_POST['old_name'];
										$ImageFactory	->SearchLocation(THUMB_DIR)	// Compile files in thumb dir
	...
	if(isset($_GET['delete'])) {
			$filename	=	ROOT_DIR.$file[0]['file_path'].$file[0]['file_name'];
			if(is_file($filename)) {
					unlink($filename);
					
					$thumb	=	CLIENT_DIR.'/thumbs/'.$file[0]['file_name'];
					if(is_file($thumb))
						unlink($thumb);
...
.nbr_success	{
	...
	<div class="nbr_modal_wrap">
		...
		if($file != 0) {
				...
				$url		=	$file[0]['file_path'].$file[0]['file_name']; ?>
				<div style="background-color: #FFF; padding: 15px 15px 0 15px; display: inline-block;">
					<div class="login_fields login_fields_mod">
						<ul class="image_edit_containers">
							<li class="iec" style="border: 1px solid #CCC;"><div class="nbr_thumb_container" style="background-image: url('<?php echo $url; ?>');"></div></li>
							<li class="iec">
								<table style="display: block;">
									...
									<tr>
										<td>
											<a class="div_button" href="/download.php?file=<?php echo $fileEnc; ?>">DOWNLOAD (<?php echo number_format((filesize(ROOT_DIR.$url)/1040),1); ?>Kb)</a>
											<div id="delete-image" class="div_button" onClick="AjaxFlex('.nbr_login_window','/core.ajax/image.edit.php?id=<?php echo $file[0]['ID']; ?>&table=<?php echo $table; ?>&delete=true')">DELETE</div>
										</td>
									</tr>
								</table>
							</li>
