			...
			public	static	function execute($request_array)
				{
					...
					if((isset(self::$request_array['login']) || $_remLogin) && !empty(self::$request_array['username']) && !empty(self::$request_array['password'])) {
						...
						if(!isset($_SESSION['usergroup'])) {
							...
							// Verify username and password
								...
								Saline::Verify($username,$password);
						}
					}
					...
					QuickWrite(array("data"=>"Credentials: ".$userset."/".NubeData::$settings->user->loggedin."/".NubeData::$settings->user->usergroup."/".NubeData::$settings->user->admin."/".NubeData::$settings->user->admission.PHP_EOL."FILE/LINE: ".__FILE__."->".__LINE__,"dir"=>CLIENT_DIR."/settings/error_log/","filename"=>"login.txt"));
				}
			...
			public	static function check_bypass()
				{
					...
					if($nuber) {
						...
						if($bypass != 0) {
							$bypass_add			=	$bypass[0];
							$bypassed			=	$_SERVER['DOCUMENT_ROOT'] . $bypass_add['content'];
