			...
			public	function __construct($_creds = array())
				{
					...
					$_classes		=	(isset($classes_dir) && !empty($classes_dir))? ROOT_DIR.$classes_dir: getcwd();
					$_database		=	(isset($dbconnect_dir) && !empty($dbconnect_dir))? ROOT_DIR.$dbconnect_dir: getcwd();
					$_root			=	(isset($root_dir) && !empty($root_dir))? ROOT_DIR.$root_dir: ROOT_DIR;
			...
			public	function headerInstructions($timezone,$errors_status = 0, $redirect)
				{
					...
					//	print_r($install->final_report);
						...
						header("Location: " . str_replace(ROOT_DIR, "", $curr_dir . '?layout=simple'));
						exit;
					endif;
					
					if($this->redirect	== true) :
						// Check to see if there is a db file
						if(is_file(ROOT_DIR.'/client_assets/settings/dbcreds.php')) {
						...
						if(!isset($_SESSION['first_run']) && (!is_file(ROOT_DIR . '/client_assets/settings/dbcreds.php'))) :
							$this->first_run	=	true;
						//	print_r($install->final_report);
        ...
        <div style=" width: 94%; min-width: 320px; margin: 0 auto; padding: 30px 3%;">
            <div id="hide_install"><?php
					...
					//	$ns_curr_type		=	$ns_log_file[2];
						...
						$my_local_log		=	ROOT_DIR . '/client_assets/update/log.txt';
                        ...
                        if(is_admin()) :
                            ...
                            if ($handle = opendir(ROOT_DIR)) {
                                ...
                                while (false !== ($entry = readdir($handle))) {
                                        if(is_dir(ROOT_DIR . "/$entry") && $entry !== '.'  && $entry !== '..') $select_root[$entry]	=	$entry;
                                    }
                ...
                <div style="display: inline-block; width: 100%;">
                    <form action="" method="post" enctype="application/x-www-form-urlencoded">
                        ...
                        <select name="install_location" style="height: 30px; font-size: 16px;">
                            <option value="<?php echo ROOT_DIR; ?>">Root Folder (perimissions: <?php echo substr(sprintf('%o', fileperms(ROOT_DIR)), -4); ?>)</option>
