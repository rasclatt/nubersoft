			...
			public	function __construct($ref_page,$ref_spot)
				{
					...
					$this->bEngine		=	false;
					$bSettings			=	array("page_live"=>"on","ref_spot"=>$this->ref_spot,"ref_page"=>$this->ref_page);//,"parent_id"=>""
...
<?php						echo $this->posts[$key]['content'];
							...
							if(is_admin()) {
								echo $this->mTools($this->posts[$key]['ID'],$this->posts[$key]['unique_id'],$this->posts[$key]['content']);
							}
...
<?php				}
			...
			public	function mTools($ID,$unique_id,$content)
				{
					ob_start();
?>				<form method="post">
					<fieldset class="">
