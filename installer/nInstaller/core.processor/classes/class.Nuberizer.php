			...
			public	function Core()
				{
					...
					if(!nApp::siteLive() && !is_admin()) {
						...
						NubeData::$settings->site->unique_id	=	(!empty(nApp::getPage('unique_id')))? nApp::getPage('unique_id'):false;
						global	$_dbrun;
						$_dbrun	=	false;
						// Grab the proper template
						include(nApp::getSite('template'));
					}
				}
							...
							for($a = 0; $a < $count; $a++) {
								...
								if($inc) {
									...
									if(!function_exists($func)) {
										if(is_file(ROOT_DIR.$array[$a]['include']))
											include(ROOT_DIR.$array[$a]['include']);
										else
			...
			protected	function fetchTemplate()
				{
					...
					elseif(isset($page->is_admin) && $page->is_admin == 1) {
						$admin		=	true;
						$template	=	(!empty($page->template))? ROOT_DIR."/".$page->template : false;
					}
					else {
						$admin		=	false;
						$template	=	(isset($page->template))? ROOT_DIR."/".$page->template : false;
					}
			...
			protected	function getAllConfigs($additional = false)
				{
					...
					$locations[]	=	ROOT_DIR.'/core.plugins/';
					$locations[]	=	CLIENT_DIR.'/plugins/';
					$locations[]	=	CLIENT_DIR.'/apps/';
