			...
			public	function Form($filename = false)
				{
					...
					$file_raw	=	str_replace(ROOT_DIR,"",$filename);
					...
					include(RENDER_LIB."/class.html/TextEditor/Form.php");
				}
			...
			public	function FileList($directory = false)
				{
					...
					$directory	=	($directory != false && !empty($directory))? $directory:CLIENT_DIR."/";
					$dir		=	get_files_folders($directory);
					// Create thumbnail previewer
					$filecheck	=	new FileMaster();
					$img_file	=	$filecheck->Initialize();
					
					include(RENDER_LIB.'/class.html/TextEditor/FileList.php');
			...
			protected	function DownloadFile($add_file = false, $filename = false, $root = false, $download = false)
				{
					...
					$filename	=	($filename == false || empty($filename))? basename($add_file):$filename;
					$root		=	($root == false || empty($root))? ROOT_DIR.'/temp':$root;
					
					$Zipper		=	new ZipEngine(ROOT_DIR);
