			...
			public	function addField($settings = false)
				{
					...
					if(is_file($input = RENDER_LIB.'/class.html/FormBuilder/addField/'.$type.'.php'))
						// Include it
						include($input);
			...
			public	function errorFeedback($var,$type = 'unknown',$is_error = false)
				{
					...
					switch($type) {
							case('url_invalid') : 
								$offending	=	(!is_file($actFile = str_replace("//","/",ROOT_DIR.'/'.$var)))? str_replace("//","/",$root.str_replace(ROOT_DIR,"",$actFile)) : false;
								$color		=	"orange";
								if($offending != false)
									$this->validation['warning'][]		=	$type;
								break;
							case('url_dynamic') :
								$offending	=	(preg_match('/\.php$|\.phtml$|\.php3$|\.asp$/i',$var))? false : str_replace("//","/",$root.$var);
								$color		=	"red";
								if($offending != false)
