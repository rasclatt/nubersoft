	...
	class ConstructMySQL implements QueryEngine
		{
			...
			public	function fetch()
				{
					...
					if($execute) {
							...
							if(preg_match('/column/i',$e->errorInfo[2])) {
								...
								if(isset($match[1])) {
									...
									try {
										...
										$tSend->execute();
										$_SESSION['q'][]	=	__METHOD__.":".__LINE__.":==>>".strtolower($showSql);
										...
										if(!empty($result) &&  count($result) == 1) {
											...
											if(is_admin($isValidTbl)) {
												...
												$_SESSION['q'][]	=	__METHOD__.":".__LINE__.":==>>".strtolower($alterSQL);
										...
										$log	=	array("data"=>date("Y-m-d H:i:s")." / sql_add_column: Authored by ".$author." / ".$useTable." / ".$match[1]." /","dir"=>CLIENT_DIR."/settings/error_log/","filename"=>"sql.alter_table.txt","mode"=>"c+");
										QuickWrite($log);
										return 0;
									}
								}
						...
						try {
							...
							$ps	=	debug_backtrace();
							if(!empty($_SESSION['q']['stmts']) && in_array($this->statement,$_SESSION['q']['stmts']))
								$_SESSION['q']['dups'][]	=	array($this->statement,$ps[0]['file'],$ps[0]['line'],$this->bind,$ps[1]['file'],$ps[1]['line']);
							else {
								// Save to query recorder
								$_SESSION['q']['stmts'][]	=	$this->statement;
								$_SESSION['q']['uni'][]		=	array($this->statement,$ps[0]['file'],$ps[0]['line'],$this->bind,$ps[1]['file'],$ps[1]['line']);
							}
							...
							if(!empty($this->statement) && is_admin()) {
								...
								QuickWrite(array("data"=>"sql: By ".$user." / ".$this->statement.PHP_EOL.PHP_EOL."FILE/LINE: ".__FILE__."->".__LINE__.PHP_EOL.strip_tags(printpre(backtrace_file(true))),"dir"=>CLIENT_DIR."/settings/error_log/","filename"=>"sql.fetch.txt"));
							}
						}
						catch (PDOException $e) {
							if(is_admin()) {
								...
								QuickWrite(array("data"=>"sql: Author->{$useNm}->".$this->statement.PHP_EOL.PHP_EOL."FILE/LINE: ".__FILE__."->".__LINE__,"dir"=>CLIENT_DIR."/settings/error_log/","filename"=>"sql.fetch_error.txt"));
							}
						}
			...
			public	function write($write = false)
				...
				/*	if($this->valid_table != true) {
						$this->errors['validate'][]	=	'Invalid Table.';
						$this->err['validate'][]	=	'Invalid Table.';
						return $this;
					}
				*/	
					try {
						...
						if(isset($this->bind) && !empty($this->bind)) {
						...
						$_SESSION['q'][]	=	__METHOD__.":".__LINE__.":==>>".$this->statement;
						...
						if(!empty($this->statement)) {
							...
							QuickWrite(array("data"=>"sql: By ".$user." / ".$this->statement.PHP_EOL.PHP_EOL."FILE/LINE: ".__FILE__."->".__LINE__,"dir"=>CLIENT_DIR."/settings/error_log/","filename"=>"sql.write.txt"));
						}
					}
					catch (PDOException $e) {
						if(is_admin()) {
			...
			protected	function checkValidTables($tablename = false)
				{
					if(!nApp::getTables()) {
						if(!isset($this->valid_tables)) {
							...
							$_SESSION['q'][]	=	__METHOD__.":".__LINE__.":==>>".$showSQL;
					...
					$thisTable			=	(in_array($this->table,$this->valid_tables));
			...
			public	function execute()
				{
					...
					$this->statement	=	(is_array($this->sql))? implode(" ", (array) $this->sql).$order.$limit:"";
			...
			public	function query($sql = false)
				{
					...
					$this->execute();
					$sql	=	(!empty($this->sql))? $this->statement : $sql;
