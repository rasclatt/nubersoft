	...
	class PageBuilder
		{
			...
			protected	$_value;
			protected	$_sql;
			protected	$_array_key;
			protected	$recurseDir;
			protected	$returnVal;
			protected	$_loop_array;
			protected	$retain_update;
			...
			public	function execute($payload = false)
				{
					...
					if(isset($this->payload['link'])) {
						if((check_empty($this->payload,'command','page_builder')) && is_admin()) {
							...
							$_isPage	=	$_SERVER['HTTP_REFERER'];

							header('Location: '.$_isPage);
							exit;
						}
					}
					else {
			...
			public	function CleanupMenu()
				{
					...
					*/
					...
					$this->payload['template']	=	(!empty($this->payload['template']) && $this->payload['template'])? $this->payload['template']: NubeData::$settings->site->template;
					...
					AutoloadFunction("menu_get_all,menu_organize_id,menu_create_dirlist");
					...
					foreach(NubeData::$settings->menu_data as $menu_arr) {										
							$menus	=	menu_create_dirlist(menu_organize_id(menu_get_all()),$menu_arr->unique_id);
							$query	->update("main_menus")
			...
			public	function Add($update = false)
				{
					...
					*/
					...
					$query			=	nQuery();
					// Columns from table
					$aKeys			=	array_keys(organize($query->describe("main_menus")->fetch(),"Field"));
					// Payload keys/values
					...
					if($valid_ui != true) {
							...
							$this->payload['unique_id']	=	FetchUniqueId();
						}
			...
			public	function BuildSiteFiles($_payload,$_key,$_value)
				{
					...
					foreach($recurseStr as $setKeys => $setValues) {
							if(is_array($setValues)) {
									// If the append directory is true, add the ROOT_DIR to the link
									$recurseDir		=	($this->doc_root == true)? ROOT_DIR."/": '/';
									...
									$doc_root	=	($this->doc_root == true)? ROOT_DIR."/": '/';
			...
			public	function PageBuilderCore($recurseDir,$returnVal,$_sql = '',$_array_key = '')
				...
				//		unlink(ROOT_DIR.'/index.php');
					...
					if(is_dir($this->recurseDir))
						...
						@copy(ROOT_DIR.'/core.processor/template/index/index.php', $this->recurseDir . 'index.php');
					
					// Update the full_path
					$_full_path		=	str_replace(array(ROOT_DIR, "index.php"), "", $this->recurseDir);
			...
			public	function Rebuild($_loop_array)
				{
					...
					*/ 
					...
					$response		=	$buildDirs->fetch($statement, 'unique_id', true, true)->response;
					
					foreach($response['info'] as $infoUnique_id => $infoObject) {
							$this->PageBuilderCore(ROOT_DIR.$infoObject['full_path'],$infoUnique_id);
						}
				}
