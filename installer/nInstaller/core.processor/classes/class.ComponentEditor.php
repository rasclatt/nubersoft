	...
	class	ComponentEditor
		{
			...
			public	function AddNewComponent($CompSet = false)
				{
					register_use(__METHOD__);
					include(RENDER_LIB.'/class.html/ComponentEditor/AddNewComponent.php');
				}
			...
			public	function Display($data = array(), $table = 'components', $allowAdd = true, $command = false)
				{
					...
					$CompSet		=	(isset($this->data['unique_id']))? true:false;
					// 
					include(RENDER_LIB.'/class.html/ComponentEditor/Display.php');
				}
			...
			public	function DeleteComponent($CompSet = false)
				{
					...
					if(isset($this->data['ID']) && !empty($this->data['ID']))
						include(RENDER_LIB.'/class.html/ComponentEditor/DeleteComponent.php');
				}
			...
			public	function TinyMCE($CompSet = false)
				{
					register_use(__METHOD__);
					if(isset($this->data['ID']) && !empty($this->data['ID'])) {
							include(ROOT_DIR.'/core.ajax/component/button.TinyMCE.php');
					 	}
				}
			...
			public	function HelpDesk($CompSet = false)
				{
					register_use(__METHOD__);
					if(isset($this->data['ID']) && !empty($this->data['ID'])) {
							include(ROOT_DIR.'/core.ajax/component/button.helpdesk.php');
					 	}
				}
			...
			public	function DuplicateComponent($CompSet = false)
				{
					register_use(__METHOD__);
					include(RENDER_LIB.'/class.html/ComponentEditor/DuplicateComponent.php');
				}
			...
			protected	function DuplicateComponentsList($values = false)
				{
					...
					if(!empty($values) && is_array($values)) {
							include(RENDER_LIB.'/class.html/ComponentEditor/DuplicateComponentsList.php');
						}
				}
			...
			public	function	ContainerDropDown()
				{
					...
					if($_checkCol != 0) {
						...
						//	$containers	=	$this->nubsql->FetchAssocArr("select unique_id,parent_id,content from " . $this->table . " where ref_page = '".$this->data['unique_id']."' and (component_type = 'div' or component_type = 'row')",'unique_id');
							...
							include(RENDER_LIB.'/class.html/ComponentEditor/ContainerDropDown.php');
						}
				}
		...
		public function createFormElements()
				{
					...
					foreach($layout as $compname => $compvals)
						include(RENDER_LIB.'/class.html/ComponentEditor/createFormElements.php');
					...
					if(isset($_SESSION) && check_empty($_SESSION,'helpdesk','on')) {
						...
						if($check_helper != 0) {
							...
							if($check_helper !==  0) {
								...
								include(ROOT_DIR.'/core.ajax/component/component.helpdesk.php');
							}
						}
					}
				}
