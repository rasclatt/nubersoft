...
/*Description: This function is the primary method of uploading files and sending file arrays back for recording to the database*/
	...
	function upload_file($settings = array())
		{		
			...
			$dir		=	(!empty($settings['dir']))? $settings['dir'] : ROOT_DIR.get_file_dir($table);
			$thumb_dir	=	(!empty($settings['thumb_dir']))? $settings['thumb_dir']:CLIENT_DIR.'/thumbs/';
			...
			if($_fileCount > 0) {
					...
					if(call_action('update')) {
							...
							$_fileName	=	(isset($fetchFile[0]['file_name']) && !empty($fetchFile[0]['file_name']))? $fetchFile[0]['file_name']:false;
