	...
	function fetch_plugins()
		{
			if(!isset(NubeData::$settings->plugins)) {
				...
				$auto				=	get_directory_list(array("dir"=>PLUGINS.'/'));
				$admin				=	get_directory_list(array("dir"=>CLIENT_DIR.'/settings/plugins/admintools/'));
				$general			=	get_directory_list(array("dir"=>CLIENT_DIR.'/settings/plugins/general/'));
				...
				if(!empty($admin_tasks)) {
					...
					for($i = 0; $i < $aCount; $i++) {
						...
						$array['admin_local'][]	=	str_replace(ROOT_DIR,"",$admin_tasks[$i]);
					}
				}
				...
				if(!empty($user_tasks)) {
					...
					for($i = 0; $i < $uTasksCnt; $i++) {
						...
						$array['user_local'][]	=	str_replace(ROOT_DIR,"",$user_tasks[$i]);
					}
				}
