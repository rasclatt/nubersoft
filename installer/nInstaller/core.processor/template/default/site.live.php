...
div.postit {
	...
	margin-left: auto;
	margin-right: auto;
	background-image: url(/core_images/core/offline.png);
			...
			<div class="admin_panel_cont">
				<?php
					...
					if($siteEnabled) {
							...
							if(nQuery()->select("COUNT(*) as count")->from("users")->fetch() == 0)
...
?>
                    ...
                    <?php if($_upass) {?>
                    <p><span style="color: red; font-style: oblique;">Wrong login information</span></p><?php } ?>
                    <form id="remote" enctype="application/x-www-form-urlencoded" method="post">
						...
						<input type="hidden" name="token[login]" value="<?php echo $token = fetch_token('login'); ?>" />
