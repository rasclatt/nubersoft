	...
	<div style="display: inline-block; float: left; margin: 5px; border: 1px solid #CCC; border-radius: 4px; padding: 10px;">
		<form enctype="multipart/form-data" method="post">
			...
			if($kind == 'pdf' || $kind == 'psd') {
					if(!is_file($mkthum = CLIENT_DIR."/thumbs/image_bucket/".$row['file_name']) && class_exists("Imagick")) {
							$imagick = new Imagick();
							$imagick->readImage(ROOT_DIR.$row['file_path'].$row['file_name']);
			...
			if(is_file(ROOT_DIR.$file)) {
					...
					if(defined("THUMB_DIR") && is_file(THUMB_DIR."/image_bucket/".$row['file_name']))
						$default_img	=	str_replace(ROOT_DIR,"",THUMB_DIR."/image_bucket/".$row['file_name']);
			...
			<?php } ?>
					...
					<td style="vertical-align: top;">
						...
						$icn 		=	(is_file(ROOT_DIR.$file_icn))? true:false;	
