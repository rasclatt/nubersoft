...
if(!empty($dir)) {
	foreach($dir as $folder => $files) {
		$afolder['local']		=	str_replace(ROOT_DIR,"",$folder);
		$afolder['id']			=	str_replace(array("/",".","_"),"",$afolder['local']);
...
?>	<tr>
		<td>
			<div class="topfolder text-editor-files btntrigger" data-button="<?php echo $afolder['id']; ?>"><img src="<?php echo site_url(); ?>/core_images/ui/folder.png" style="max-height: 20px; float: left; display: inline-block;" /><?php echo $afolder['local']; echo " (".count($files).")"; ?></div>
...
<?php		if(!empty($thumbnail->layout))
				...
				$useIcon	=	(is_file(ROOT_DIR.$icn))? $icn : $def_core;
...
?>							<p><a href="<?php echo site_url().str_replace(ROOT_DIR,"",$file); ?>" target="_blank"><?php echo basename($file); ?></a></p>
<?php		}
...
<?php 		if($readable) {
?>							<p style="font-size: 10px;"><?php echo str_replace(ROOT_DIR,"",$file); ?></p>
<?php 		}
?>						</td>
					</tr>
				</table>
