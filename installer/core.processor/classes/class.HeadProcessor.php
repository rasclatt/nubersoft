			...
			public	function Process($payload = false)
				{
					if(!nApp::siteValid()) {
						...
						if(!empty(nApp::getPost('save_credentials')) && is_admin()) {
							if(!nApp::tokenMatch('dbinstall'))
								return;
								
							include_once(NBR_RENDER_LIB.'/admintools/classes/installer/create.creds.php');
					...
					if(!empty($this->payload['requestTable']) && !empty($nubquery)) {
						...
						// Process token
						if(!nApp::tokenExists('nProcessor')) {
							nApp::saveIncidental('token_error', array('error'=>'no token found'));
							return false;
						}
						else {
							if(!nApp::tokenMatch('nProcessor'))
								return false;
						}
