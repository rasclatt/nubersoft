		...
		public	function makeExpireBar($settings = false)
			{
				...
				// Javascript setting
				$expire		=	(isset($settings['expire']))? $settings['expire'] : nApp::getSessExpTime();
				...
				$onClick	=	(!empty($settings['on_click']))? $settings['on_click'] : site_url().nApp::getPage('full_path');
...
?>	// See user is logged in
	var is_loggedout	=	(<?php echo $this->jName; ?>(".fullsite").html() == undefined);
...
<?php			$data	=	ob_get_contents();
		...
		public	function saveDocument($name = '/client_assets/js/myScripts.js')
			{
				// File path
				...
				if(!nApp::dirExists($path['dirname'])) {
					// Try making it
					if(!nApp::dirExists($path['dirname'],true))
						return false;
					else {
						// Load the create file
						AutoloadFunction('CreateHTACCESS');
						// Create htaccess file
						CreateHTACCESS(array('rule'=>'server_r','dir'=>$path['dirname']));
						// If not made register error
						if(!file_exists($path['dirname'].'/.htaccess'))
							nApp::saveIncidental('htaccess_write',array('error'=>'Failed to write htaccess file. Possibly permissions.'));
					}
				}
				...
				$content	=	trim(rtrim(ltrim(trim(Safe::decode($this->getResults())),'<script>'),'</script>'));

				AutoloadFunction("write_file");
				write_file(array('content'=>$content,'save_to'=>$build));
				
				if(is_file($build))
					nApp::saveIncidental('javascript_write',array('success'=>true));
			}
	}